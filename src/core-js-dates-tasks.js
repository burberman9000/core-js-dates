...
function getDayName(date) {
  ...
  const dataBase = new Date(date).getDay();
  return weeks[dataBase];
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  const timeDiff = new Date(dateEnd) - new Date(dateStart);
  const result = timeDiff / (1000 * 60 * 60 * 24) + 1;
  return result;
}
...
function getNextFridayThe13th(date) {
  ...
  while (day.getDate() !== 13 || day.getDay() !== 5) {
...
const leapYear = new Date(2024, 2, 1);
isLeapYear(leapYear);
